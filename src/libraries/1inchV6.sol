        ...
        if (desc.flags & _PARTIAL_FILL != 0) {
            ...
            if (returnAmount * desc.amount < desc.minReturnAmount * spentAmount)
                revert RouterErrors.ReturnAmountIsNotEnough(
                    returnAmount,
                    (desc.minReturnAmount * spentAmount) / desc.amount
                );
        } else {
...
contract UnoswapRouter is Pausable, EthReceiver, IUniswapV3SwapCallback {
    ...
    bytes32 private constant _CURVE_COINS_SELECTORS =
        0x87cb4f5723746eb8c6610657b739953eb9947eb0000000000000000000000000;
    ...
    function _curfe(address recipient, uint256 amount, uint256 minReturn, Address dex) private returns (uint256 ret) {
        ...
        assembly ("memory-safe") {
            ...
            if and(iszero(hasCallback), or(iszero(useEth), toTokenIsEth)) {
                let fromSelectorOffset := and(
                    shr(_CURVE_FROM_COINS_SELECTOR_OFFSET, dex),
    ...
    function uniswapV3SwapCallback(
        ...
        assembly ("memory-safe") {
            ...
            default {
                ...
                case 1 {
                    if iszero(
                        staticcall(
                            gas(),
                            caller(),
                            add(emptyPtr, _TOKEN1_SELECTOR_OFFSET),
                            0x4,
                            add(resultPtr, 0x20),
                            0x20
                        )
                ...
                default {
                    if iszero(
                        staticcall(gas(), caller(), add(emptyPtr, _TOKEN0_SELECTOR_OFFSET), 0x4, resultPtr, 0x20)
                    ) {
                        reRevert()
                    }
                }
